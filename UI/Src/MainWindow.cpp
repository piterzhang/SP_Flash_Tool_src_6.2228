...
void MainWindow::setupBeforeCmdOperation(CONN_END_STAGE conn_end_strage, bool initial_dram/* = true*/)
{
    ...
    if (conn_end_strage == CONN_END_STAGE::SECOND_DA_STAGE) {
        ...
        if (Utils::isSupportDevTracking(ui->comboBox_flash_xml_file->currentText().trimmed())) {
            if (NetworkInft::instance()->network_online()) {
                m_backgroundworker->setNeedTrackingDevice(true);
            } else {
                m_backgroundworker->setNeedTrackingDevice(false);
                NetworkInft::instance()->tryConnect();
            }
        }
#endif

        if (OptionIniFileMgr::getInstance()->getCheck_storage_life_cycle()) {
...
#ifdef _INTERNAL_PHONE_TRACKING_MODE
    connect(NetworkInft::instance().get(), &NetworkInft::signal_net_connection_changed, this, &MainWindow::slot_show_network_connected);
#endif
}
...
void MainWindow::setUIInitState()
{
    ...
    ui->actionDownload_Cert->setVisible(false);
...
void MainWindow::updateNetworkActionVisible()
{
#ifdef _INTERNAL_PHONE_TRACKING_MODE
    if (Utils::isSupportDevTracking(ui->comboBox_flash_xml_file->currentText().trimmed())) {
        ...
        ui->actionNetwork_Test->setVisible(false);
        m_download_widget->setNetLabelVisible(false);
    }
#endif
}
...
void MainWindow::slot_setMenuItemStatus()
{
    ...
    ui->actionDownload_Cert->setEnabled(isLoad && m_dl_cert_on
                                        && m_dl_cert_widget->validBeforeDLCert().first);
}
...
void MainWindow::toggleDLCertWidget()
{
    ...
    ui->tabWidget->setUpdatesEnabled(true);
    ui->actionDownload_Cert->setVisible(m_dl_cert_on);
}
...
void MainWindow::on_actionDownload_Cert_triggered()
{
    exportConfigXMLFile("download_cert.xml", std::bind(&DLCertWidget::createCmdWrapperList, m_dl_cert_widget),
                        std::bind(&DLCertWidget::getConnStage, m_dl_cert_widget));
}
...
void MainWindow::slot_after_check_storage_life_cycle_cmd(int *p_result, const std::shared_ptr<Connection> conn,
                                                         const std::shared_ptr<ICmdXMLGenerator> iCmdXMLGenerator)
{
    *p_result = ICmdCallBack::cb_after_check_storage_life_cycle_cmd(conn, iCmdXMLGenerator);
    if (*p_result == CFT_CHECK_STORAG_LIFE_CYCLE_WARNING) {
        ...
        if (ret == QMessageBox::AcceptRole) {
            ...
            *p_result = CFT_CHECK_STORAG_LIFE_CYCLE_WARN_CANCLE;
        }
    }
}
...
void MainWindow::slot_show_network_connected(NetworkInft::network_con_changed_src con_changed_src)
{
    m_download_widget->setNetLabelVisible(NetworkInft::instance()->network_online());
    if (con_changed_src == NetworkInft::nccs_try_connect_show) {
        if (NetworkInft::instance()->network_online()) {
            Utils::flashtool_message_box(this, nullptr, INFORMATION_MSGBOX, STRING_TOOL_NAME,
                                         "Connection successed", STRING_OK);
        } else {
            Utils::flashtool_message_box(this, nullptr, WARNING_MSGBOX, STRING_TOOL_NAME,
                                         "Connection failed", STRING_OK);
        }
    }
}
