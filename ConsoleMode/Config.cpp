...
Config::Config(const std::shared_ptr<CommandLineArguments> &cmd_line_args)
{
    ...
    else
    {
        ...
        loadFile(cmd_line_args->getConfigXMLFileName(), cmd_line_args->reboot_to_atm(),
                 cmd_line_args->needStorageLifeCycleCheck());
...
void Config::loadFile(const QString &xml_file_name, bool reboot_to_atm, bool need_storage_life_chk)
{
    ...
    if (!m_command_setting) {
        m_command_setting = std::make_shared<CommandSetting>(reboot_to_atm,
                                                             m_general_setting->getFlashXmlFile(),
                                                             need_storage_life_chk);
    }
