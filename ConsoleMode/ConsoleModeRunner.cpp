...
void ConsoleModeRunner::execute(const Config &config, const ConsoleModeNetworkIntf *p_consoleModeNetworkIntf) const
{
#ifndef _INTERNAL_PHONE_TRACKING_MODE
    Q_UNUSED(p_consoleModeNetworkIntf)
#endif
    ...
    if (p_consoleModeNetworkIntf) {
        p_consoleModeNetworkIntf->postData(conn, cbs.trans);
    }
#endif
...
void ConsoleModeRunner::cbDLCertProgress(void *_this, unsigned int progress, unsigned long long data_xferd, const QString &info)
{
    Q_UNUSED(_this)
    Q_UNUSED(info)
    Q_UNUSED(data_xferd)
    LOGI("download cert file: %d%%.\n", progress);
}
