...
QMap<QString, QStringList> CommandSetting::m_cmd_split_map = {
    ...
    {DL_CERT_CMD_NAME, {WRITE_PRIVATE_CERT_CMD_NAME_VAL}}
};
...
CommandSetting::CommandSetting():
    ...
    m_is_firmware_upgrade_op(false),
    m_need_storage_life_check(true)
{
    registerCmdCreator();
}

CommandSetting::CommandSetting(bool reboot_to_atm, const QString &flash_xml_file, bool need_storage_life_chk):
    m_cmd_args(nullptr),
    ...
    m_is_firmware_upgrade_op(false),
    m_need_storage_life_check(need_storage_life_chk)
{
    registerCmdCreator();
}
...
CommandSetting::CommandSetting(const std::shared_ptr<CommandLineArguments> &args):
    ...
    m_is_firmware_upgrade_op(false),
    m_need_storage_life_check(args->needStorageLifeCycleCheck())
{
    registerCmdCreator();
...
void CommandSetting::loadXML(const QDomElement &xml_node)
{
    ...
    while (!command_node.isNull()) {
        ...
        if ((command_name == Add_CMD_PREFIX(LIFE_CYCLE_CHECK)) && !m_need_storage_life_check) {
            command_node = command_node.nextSiblingElement();
            continue;
        }
        if (this->isDownloadCmd(command_name)) {
            m_has_download_cmd = true;
            m_is_firmware_upgrade_op = command_name == FLASH_UPDATE_EX_CMD_NAME_VAL;
        }
...
void CommandSetting::slot_after_check_storage_life_cycle_cmd(int *p_result, const std::shared_ptr<Connection> conn, const std::shared_ptr<ICmdXMLGenerator> iCmdXMLGenerator)
{
    *p_result = ConsoleModeCallBack::cb_after_check_storage_life_cycle_cmd(conn, iCmdXMLGenerator);
    if (*p_result == CFT_CHECK_STORAG_LIFE_CYCLE_WARNING) {
        LOGW("90%% of life time for storage in your device has passed, continue to run!");
        *p_result = CFT_SUCCESS; // for console mode, only stopped operation when device storage EXHAUSTED.
    }
}
...
void CommandSetting::slot_after_get_DL_img_feedback_cmd(int *p_result, const std::shared_ptr<Connection> conn, const std::shared_ptr<ICmdXMLGenerator> iCmdXMLGenerator)
{
    *p_result = ICmdCallBack::cb_after_get_dl_img_feedback_cmd(conn, iCmdXMLGenerator, getLoadChksumFile(),
                                                               getScatterFile(), m_is_firmware_upgrade_op);
}
...
void CommandSetting::registerCmdCreator()
{
    m_cmd_creator_map = {
        ...
        {WRITE_PRIVATE_CERT_CMD_NAME_VAL, std::bind(&CommandSetting::createDLCertCmdWrapper, this)}
    };
}
...
void CommandSetting::createCmdSettingFromArgs()
{
    ...
    foreach(auto split_cmd_name, split_cmd_names) {
        ...
        if (this->isDownloadCmd(split_cmd_name)) {
            m_has_download_cmd = true;
            m_is_firmware_upgrade_op = split_cmd_name == FLASH_UPDATE_EX_CMD_NAME_VAL;
        }
...
cmd_wrapper_ptr CommandSetting::createDLCertCmdWrapper() const
{
    std::shared_ptr<WritePrivateCertCmdXML> writeCertCmdXML = std::make_shared<WritePrivateCertCmdXML>();
    if (m_cmd_args && m_cmd_args->getCurrentMode() == ConsoleModeCurrentMode::CmdArgumentMode) {
        writeCertCmdXML->setFileName(m_cmd_args->getFile());
    }
    std::shared_ptr<CmdWrapper> cmd_wrapper = std::make_shared<CmdWrapper>(writeCertCmdXML);
    cmd_wrapper->setOperationProgress(ConsoleModeRunner::cbDLCertProgress);
    return  cmd_wrapper;
}
