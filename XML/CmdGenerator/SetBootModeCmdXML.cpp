...
void MetaBootModeCmdXML::loadXML(const QDomElement &xml_node)
{
    ...
    m_mobile_log_on = Utils::transOnOffToBool(value);
    value = xml_node.attribute(QStringLiteral("adb"), QStringLiteral("off"));
    m_adb_on = Utils::transOnOffToBool(value);
}
...
void MetaBootModeCmdXML::saveXML(QDomDocument &xml_doc, QDomElement &xml_node) const
{
    ...
    set_boot_mode_node.setAttribute(QStringLiteral("mobile_log"), Utils::transBoolToOnOff(m_mobile_log_on));
    set_boot_mode_node.setAttribute(QStringLiteral("adb"), Utils::transBoolToOnOff(m_adb_on));
}
...
void MetaBootModeCmdXML::fillArgNode(QDomElement * const arg_node)
{
    ...
    this->createXMLSubNode(arg_node, MOBILE_LOG_NODE_NAME, Utils::transBoolToOnOff(m_mobile_log_on));
    this->createXMLSubNode(arg_node, ADB_NODE_NAME, Utils::transBoolToOnOff(m_adb_on));
}
