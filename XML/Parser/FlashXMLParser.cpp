...
bool FlashXMLParser::getEnablePhoneTracking() const
{
    return m_enable_phone_tracking;
}
...
bool FlashXMLParser::parseDocument(const QDomElement &root_node)
{
    ...
    if (!parse_result) return parse_result;
    m_contact = value;
    parse_result = parseNodeValue(value, root_node, ENABLE_PHONE_TRACKING_NAME, false);
    if (!parse_result) return parse_result;
    m_enable_phone_tracking = value == QStringLiteral("Y") ? true : false;
    return parse_result;
}
