...
QSet<QString> ScatterXMLParser::get_protected_part_set() const
{
    return m_protected_part_set;
}

bool ScatterXMLParser::parseDocument(const QDomElement &root_node)
{
    m_protected_part_set.clear();
                                               ...
                                               const QDomElement &partition_index_node)
{
    ...
    QDomElement op_type_node = partition_index_node.firstChildElement(OP_TYPE_NODE_NAME);
    if (op_type_node.isNull()) {
        m_error_msg = this->genNoChilElementError(partition_index_node, OP_TYPE_NODE_NAME);
        return false;
    }

    QString op_type_val = op_type_node.text().trimmed();
    static QSet<QString> protected_type_set({BINREGION_TYPE, PROTECTED_TYPE});
    if (protected_type_set.contains(op_type_val)) {
        m_protected_part_set.insert(partition_name);
    }
