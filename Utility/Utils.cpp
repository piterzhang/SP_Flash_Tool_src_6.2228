...
bool Utils::isSupportDevTracking(const QString &flash_xml_file)
{
    if(ToolInfo::IsCustomerVer()) {
        return false;
    }

    if (!flash_xml_file.isEmpty()) {
        FlashXMLParser flashxmlparser;
        if (flashxmlparser.parseXMLFile(flash_xml_file)) {
            return flashxmlparser.getEnablePhoneTracking();
        } else {
            LOGE(qPrintable(flashxmlparser.get_error_msg()));
        }
    }
    return false;
}
